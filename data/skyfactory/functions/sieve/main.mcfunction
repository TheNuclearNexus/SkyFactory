...
execute if block ~ ~ ~ #du:air run data merge entity @e[type=item,nbt={Item:{id:"minecraft:chest"}},sort=nearest,limit=1,distance=..1] {Item:{id:"minecraft:ghast_spawn_egg",tag:{display:{Name:"{\"translate\":\"sf.sieve\",\"italic\":false}"},CustomModelData:405,EntityTag:{id:"minecraft:armor_stand",Small:1b,Marker:1b,Invisible:1b,Tags:["sf.placeSieve","sf.block"]}}}}
execute if block ~ ~ ~ #du:air run kill @s
